...
bool Application::init(){
 if(SDL_Init(SDL_INIT_VIDEO|SDL_INIT_EVENTS|SDL_INIT_TIMER)!=0){ std::fprintf(stderr,"SDL_Init failed: %s\n", SDL_GetError()); return false; }
 int flags=IMG_INIT_PNG; if((IMG_Init(flags)&flags)!=flags){ std::fprintf(stderr,"IMG_Init failed: %s\n", IMG_GetError()); return false; }
 ...
 m_posX=m_width*0.5f; m_posY=m_height*0.5f;
 Input::init();
 m_tex.loadFromFile(m_renderer,"assets/checker.png");
 if(m_atlas.loadGrid(m_renderer,"assets/atlas8x1.png",32,32,0,0)){ m_anim.set(m_atlas.frameCount(), 8.0f, true); }
 return true;
}
void Application::shutdown(){
 delete m_r2d; m_r2d=nullptr; m_tex.destroy(); Input::shutdown();
...
void Application::update(double dt){
 ...
 if(m_follow){ m_cam.x=m_posX - m_width*0.5f/m_cam.zoom; m_cam.y=m_posY - m_height*0.5f/m_cam.zoom; }
 else { m_cam.x=0.f; m_cam.y=0.f; }
 m_time+=dt;
}
void Application::render(){
 ...
 if(fr){ m_r2d->drawTextureRegion(m_atlas.texture(), *fr, m_posX, m_posY, m_scale, m_rot); }
 else if(m_tex.sdl()){ m_r2d->drawTexture(m_tex, m_posX, m_posY, m_scale, m_rot); }
 m_r2d->present();
}
int Application::run(){
 if(!init()){ shutdown(); return -1; }
 bool running=true; Uint64 f=SDL_GetPerformanceFrequency(), last=SDL_GetPerformanceCounter();
